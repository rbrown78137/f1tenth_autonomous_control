...
class CustomImageDataset(Dataset):
    def __init__(self, filePath, transform=None, target_transform =None):
        ...
        self.labels = torch.zeros([int(self.data.__len__()/(steeringAngleByteSize*numberOfDataPointsStored)),3], dtype=torch.float)
        ...
        for idx in range(int(self.data.__len__()/(steeringAngleByteSize*numberOfDataPointsStored))):
            unmodifiedCameraImage = cv.imread('/home/ryan/TrainingData/simulator_turning_direction/photos/' + str(idx) + "-semanticOutput.png", cv.IMREAD_GRAYSCALE)
            ...
            inputLaneState = struct.unpack('i', self.data[(steeringAngleByteSize*numberOfDataPointsStored)*idx:steeringAngleByteSize+(steeringAngleByteSize*numberOfDataPointsStored)*idx])[0]
            ...
            if round(inputLaneState) == 0:
                self.labels[idx] = torch.tensor([0, 0, 0])
            if round(inputLaneState) == 1:
                self.labels[idx] = torch.tensor([0, 1, 0])
            if round(inputLaneState) == 2:
                self.labels[idx] = torch.tensor([0, 0, 1])
...
dataset = CustomImageDataset("/home/ryan/TrainingData/simulator_turning_direction/turningDirection.bin")
'''
